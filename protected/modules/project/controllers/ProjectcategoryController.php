<?php
/* ProjectCategoryController* Handle ProjectCategoryController* Copyright (c) 2012, SWEVEL. All rights reserved.
* version: 0.0.1
* Reference start
*
* TOC :
...
class ProjectcategoryController extends /*SBaseController*/ Controller
 {
	/**
	...
	public function actionIndex() {
		...
		$dataProvider=new CActiveDataProvider('ProjectCategory', array(
			//'criteria' => $criteria,
			//'pagination' => array(
			//	'size' => 10
			//)
	...
	public function actionAdminAdd() {
		...
		if(isset($_POST['ProjectCategory'])) {
			...
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'ProjectCategory success created.'));
	...
	public function actionAdminEdit($id) {
		...
		if(isset($_POST['ProjectCategory'])) {
			$model->attributes=$_POST['ProjectCategory'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'ProjectCategory success updated.'));
				$this->redirect(array('adminview','id'=>$model->id));
				//$this->redirect(array('adminmanage'));
			}
		}
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				...
				echo CJSON::encode(array(
					'type' => 3,
					'id' => 'partial-project-category',
					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'ProjectCategory success deleted.').'</strong></div>',
			...
			$data .= '<div class="dialog-header">'.Yii::t('', 'Hapus ProjectCategory').'</div>';
