<?php
/* TimeLimitController* Handle TimeLimitController* Copyright (c) 2012, SWEVEL. All rights reserved.
* version: 0.0.1
* Reference start
*
* TOC :
...
class TimelimitController extends /*SBaseController*/ Controller
 {
	/**
	...
	public function actionIndex() {
		...
		$dataProvider=new CActiveDataProvider('TimeLimit', array(
			//'criteria' => $criteria,
			//'pagination' => array(
			//	'size' => 10
			//)
	...
	public function actionAdminManage() {
		$model=new TimeLimit('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['TimeLimit'])) {
			$model->attributes=$_GET['TimeLimit'];
		}
	...
	public function actionAdminAdd() {
		...
		if(isset($_POST['TimeLimit'])) {
			$model->attributes=$_POST['TimeLimit'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'TimeLimit success created.'));
	...
	public function actionAdminEdit($id) {
		...
		if(isset($_POST['TimeLimit'])) {
			$model->attributes=$_POST['TimeLimit'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'TimeLimit success updated.'));
				$this->redirect(array('adminview','id'=>$model->id));
				//$this->redirect(array('adminmanage'));
			}
		}
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				...
				echo CJSON::encode(array(
					'type' => 3,
					'id' => 'partial-time-limit',
					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'TimeLimit success deleted.').'</strong></div>',
			...
			$data .= '<div class="dialog-header">'.Yii::t('', 'Hapus TimeLimit').'</div>';
	...
	public function loadModel($id) {
		$model=TimeLimit::model()->findByPk($id);
		if($model===null)
	...
	protected function performAjaxValidation($model) {
		if(isset($_POST['ajax']) && $_POST['ajax']==='time-limit-form') {
