<?php
/* InstitutionSomeController* Handle InstitutionSomeController* Copyright (c) 2012, SWEVEL. All rights reserved.
* version: 0.0.1
* Reference start
*
* TOC :
...
class InstitutionsomeController extends /*SBaseController*/ Controller
 {
	/**
	...
	public function accessRules() {
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('adminmanage','adminadd','adminedit','adminview','admindelete','publish','GetIdUsername'),
				'users'=>array('@'),
			),
	...
	public function actionIndex() {
		...
		$dataProvider=new CActiveDataProvider('InstitutionSome', array(
			//'criteria' => $criteria,
			//'pagination' => array(
			//	'size' => 10
			//)
	...
	public function actionAdminManage() {
		$model=new InstitutionSome('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['InstitutionSome'])) {
			$model->attributes=$_GET['InstitutionSome'];
		}
	...
	public function actionAdminAdd() {
		$model=new InstitutionSome;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['InstitutionSome'])) {
			$model->attributes=$_POST['InstitutionSome'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'InstitutionSome success created.'));
	...
	public function actionAdminEdit($id) {
		...
		if(isset($_POST['InstitutionSome'])) {
			$model->attributes=$_POST['InstitutionSome'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'InstitutionSome success updated.'));
				$this->redirect(array('adminview','id'=>$model->id));
				//$this->redirect(array('adminmanage'));
			}
		}
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				...
				echo CJSON::encode(array(
					'type' => 3,
					'id' => 'partial-institution-some',
					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'InstitutionSome success deleted.').'</strong></div>',
			...
			$data .= '<div class="dialog-header">'.Yii::t('', 'Hapus InstitutionSome').'</div>';
    ...
    public function actionGetIdUsername(){
        if($_GET['term']){
            echo InstitutionSome::model()->getIdUserJson($_GET['term']);
        }
    }
	...
	public function loadModel($id) {
		$model=InstitutionSome::model()->findByPk($id);
		if($model===null)
	...
	protected function performAjaxValidation($model) {
		if(isset($_POST['ajax']) && $_POST['ajax']==='institution-some-form') {
