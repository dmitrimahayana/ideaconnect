<?php
/* ZoneCountryController* Handle ZoneCountryController* Copyright (c) 2012, SWEVEL. All rights reserved.
* version: 0.0.1
* Reference start
*
* TOC :
...
class ZonecountryController extends /*SBaseController*/ Controller
 {
	/**
	...
	public function actionIndex() {
		...
		$dataProvider=new CActiveDataProvider('ZoneCountry', array(
			//'criteria' => $criteria,
			//'pagination' => array(
			//	'size' => 10
			//)
	...
	public function actionAdminManage() {
		$model=new ZoneCountry('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['ZoneCountry'])) {
			$model->attributes=$_GET['ZoneCountry'];
		}
	...
	public function actionAdminAdd() {
		$model=new ZoneCountry;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['ZoneCountry'])) {
			$model->attributes=$_POST['ZoneCountry'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'ZoneCountry success created.'));
				$this->redirect(array('adminview','id'=>$model->code));
				//$this->redirect(array('adminmanage'));
			}
		}
	...
	public function actionAdminEdit($ID) {
		...
		if(isset($_POST['ZoneCountry'])) {
			$model->attributes=$_POST['ZoneCountry'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'ZoneCountry success updated.'));
				$this->redirect(array('adminview','id'=>$model->code));
				//$this->redirect(array('adminmanage'));
			}
		}
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($ID)) {
				...
				echo CJSON::encode(array(
					'type' => 3,
					'id' => 'partial-zone-country',
					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'ZoneCountry success deleted.').'</strong></div>',
			...
			$data .= '<div class="dialog-header">'.Yii::t('', 'Hapus ZoneCountry').'</div>';
	...
	public function loadModel($ID) {
		$model=ZoneCountry::model()->findByPk($ID);
		if($model===null)
	...
	protected function performAjaxValidation($model) {
		if(isset($_POST['ajax']) && $_POST['ajax']==='zone-country-form') {
