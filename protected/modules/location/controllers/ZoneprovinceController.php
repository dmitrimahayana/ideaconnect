<?php
/* ZoneProvinceController* Handle ZoneProvinceController* Copyright (c) 2012, SWEVEL. All rights reserved.
* version: 0.0.1
* Reference start
*
* TOC :
...
class ZoneprovinceController extends /*SBaseController*/ Controller
 {
	/**
	...
	public function actionIndex() {
		...
		$dataProvider=new CActiveDataProvider('ZoneProvince', array(
			//'criteria' => $criteria,
			//'pagination' => array(
			//	'size' => 10
			//)
	...
	public function actionAdminManage() {
		$model=new ZoneProvince('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['ZoneProvince'])) {
			$model->attributes=$_GET['ZoneProvince'];
		}
	...
	public function actionAdminAdd() {
		$model=new ZoneProvince;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['ZoneProvince'])) {
			$model->attributes=$_POST['ZoneProvince'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'ZoneProvince success created.'));
	...
	public function actionAdminEdit($id) {
		...
		if(isset($_POST['ZoneProvince'])) {
			$model->attributes=$_POST['ZoneProvince'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'ZoneProvince success updated.'));
				$this->redirect(array('adminview','id'=>$model->id));
				//$this->redirect(array('adminmanage'));
			}
		}
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				...
				echo CJSON::encode(array(
					'type' => 3,
					'id' => 'partial-zone-province',
					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'ZoneProvince success deleted.').'</strong></div>',
			...
			$data .= '<div class="dialog-header">'.Yii::t('', 'Hapus ZoneProvince').'</div>';
	...
	public function loadModel($id) {
		$model=ZoneProvince::model()->findByPk($id);
		if($model===null)
	...
	protected function performAjaxValidation($model) {
		if(isset($_POST['ajax']) && $_POST['ajax']==='zone-province-form') {
