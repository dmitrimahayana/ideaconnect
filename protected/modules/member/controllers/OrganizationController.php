...
class OrganizationController extends Controller
{
	...
	public function actionIndex()
	{
		...
		$dataProvider = new CActiveDataProvider('CcnJobseekerOrg', array(
			'criteria'=>$criteria,
		));
	...
	{
		$model=new CcnJobseekerOrg;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['CcnJobseekerOrg']))
		{
			$model->attributes=$_POST['CcnJobseekerOrg'];
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							'type' => 3,
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'Organisasi '.$model->org_name.' berhasil ditambahkan.').'</strong></div>',
			...
			$message['id'] = 'ccn-jobseeker-org-form';
			...
			if(strlen($jsonError) > 2) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							'type' => 3,
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'Organisasi '.$model->org_name.' berhasil diubah.').'</strong></div>',
							'get' => Yii::app()->controller->createUrl('index',array('type'=>'ajax'))
						));
					} else {
						print_r($model->getErrors());
	...
	public function actionDelete($id)
	{
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($_GET['id'])) {
				...
				echo CJSON::encode(array(
					'type' => 3,
					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'Organisasi berhasil dihapus.').'</strong></div>',
					'get' => Yii::app()->controller->createUrl('index',array('type'=>'ajax'))
				));
			}
			...
			$data .= '<div class="dialog-header">Hapus Organisasi</div>';
	...
	{
		$model=CcnJobseekerOrg::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='ccn-jobseeker-org-form')
		{
