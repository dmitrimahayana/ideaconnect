<?php
/* MessegeController* Handle MessegeController* Copyright (c) 2012, SWEVEL. All rights reserved.
* version: 0.0.1
* Reference start
*
* TOC :
...
class MessegeController extends /*SBaseController*/ Controller
 {
	/**
	...
	public function accessRules() {
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('adminmanage','adminadd','adminedit','adminview','admindelete','publish','adminreply','SentItem' ),
				'users'=>array('@'),
			),
	...
	public function actionIndex() {
		...
		$dataProvider=new CActiveDataProvider('Messege', array(
			//'criteria' => $criteria,
			//'pagination' => array(
			//	'size' => 10
			//)
	...
	public function actionAdminAdd() {
		...
		if(isset($_POST['Messege'])) {
			$model->attributes=$_POST['Messege'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'Messege success created.'));
    ...
    public function actionAdminReply($id,$subject){
        $modelData=$this->loadModelGroupMessage($id);
        $model=new Messege;
        if(isset($_POST['Messege'])) {
            $model->attributes=$_POST['Messege'];
            $model->subject=$subject;
            $model->from_user_id=8;
            $model->to_user_id=$id;
            $model->sent_time=date("Y-m-d H:i:s");
            if($model->save()) {
                Yii::app()->user->setFlash('success', Yii::t('', 'Messege admin success created.'));
                $this->redirect(array('adminview','id'=>$model->id));
                //$this->redirect(array('adminmanage'));
            }
            else {
	...
	public function actionAdminEdit($id) {
		...
		if(isset($_POST['Messege'])) {
			$model->attributes=$_POST['Messege'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'Messege success updated.'));
				$this->redirect(array('adminview','id'=>$model->id));
				//$this->redirect(array('adminmanage'));
			}
		}
...
//				$this->loadModel($id)->delete();
                $model=$this->loadModel($id);
                $model->is_deleted_by_receiver=1;
                if($model->update()) {
...
//					'type' => 3,
//					'id' => 'partial-messege',
//					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'Messege success deleted.').'</strong></div>',
...
//				));
			...
			$data .= '<div class="dialog-header">'.Yii::t('', 'Hapus Messege').'</div>';
	 ...
	 */
    public function loadModelGroupMessage($id) {
        $criteria=new CDbCriteria;
//        $criteria->condition = 'from_user_id = '.$id.' and subject="'.$subject.'"';
//        $criteria->condition = 'from_user_id = '.$id.' or from_user_id=8';
//        $criteria->order = 'sent_time desc';
//        $model=Messege::model()->findAll($criteria);
        $sql='
        SELECT * FROM `ic_messege` WHERE `to_user_id`=8 AND `from_user_id`='.$id.' and is_deleted_by_sender=0 and is_deleted_by_receiver=0
        UNION ALL
        SELECT * FROM `ic_messege` WHERE `to_user_id`='.$id.' AND `from_user_id`=8 and is_deleted_by_sender=0 and is_deleted_by_receiver=0
...
//        die();
	...
	public function loadModel($id) {
		$model=Messege::model()->findByPk($id);
		if($model===null)
			throw new CHttpException(404,'The requested page does not exist.');
		return $model;
	}
	...
	protected function performAjaxValidation($model) {
		if(isset($_POST['ajax']) && $_POST['ajax']==='messege-form') {
