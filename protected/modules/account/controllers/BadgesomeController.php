<?php
/* BadgeSomeController* Handle BadgeSomeController* Copyright (c) 2012, SWEVEL. All rights reserved.
* version: 0.0.1
* Reference start
*
* TOC :
...
class BadgesomeController extends /*SBaseController*/ Controller
 {
	/**
	...
	public function actionIndex() {
		...
		$dataProvider=new CActiveDataProvider('BadgeSome', array(
			//'criteria' => $criteria,
			//'pagination' => array(
			//	'size' => 10
			//)
	...
	public function actionAdminManage() {
		$model=new BadgeSome('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['BadgeSome'])) {
			$model->attributes=$_GET['BadgeSome'];
		}
	...
	public function actionAdminAdd() {
		...
		if(isset($_POST['BadgeSome'])) {
			$model->attributes=$_POST['BadgeSome'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'BadgeSome success created.'));
	...
	public function actionAdminEdit($id) {
		...
		if(isset($_POST['BadgeSome'])) {
			$model->attributes=$_POST['BadgeSome'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'BadgeSome success updated.'));
				$this->redirect(array('adminview','id'=>$model->id));
				//$this->redirect(array('adminmanage'));
			}
		}
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				...
				echo CJSON::encode(array(
					'type' => 3,
					'id' => 'partial-badge-some',
					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'BadgeSome success deleted.').'</strong></div>',
			...
			$data .= '<div class="dialog-header">'.Yii::t('', 'Hapus BadgeSome').'</div>';
    ...
    public function actionGetIdUsername(){
        if($_GET['term']){
            echo BadgeSome::model()->getIdUserJson($_GET['term']);
        }
    }
	...
	public function loadModel($id) {
		$model=BadgeSome::model()->findByPk($id);
		if($model===null)
	...
	protected function performAjaxValidation($model) {
		if(isset($_POST['ajax']) && $_POST['ajax']==='badge-some-form') {
