<?php
/* UsersGroupController* Handle UsersGroupController* Copyright (c) 2012, SWEVEL. All rights reserved.
* version: 0.0.1
* Reference start
*
* TOC :
...
class UsersgroupController extends /*SBaseController*/ Controller
 {
	/**
	...
	public function actionIndex() {
		...
		$dataProvider=new CActiveDataProvider('UsersGroup', array(
			//'criteria' => $criteria,
			//'pagination' => array(
			//	'size' => 10
			//)
	...
	public function actionAdminAdd() {
		...
		if(isset($_POST['UsersGroup'])) {
			...
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'UsersGroup success created.'));
	...
	public function actionAdminEdit($id) {
		...
		if(isset($_POST['UsersGroup'])) {
			$model->attributes=$_POST['UsersGroup'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'UsersGroup success updated.'));
				$this->redirect(array('adminview','id'=>$model->id));
				//$this->redirect(array('adminmanage'));
			}
		}
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				...
				echo CJSON::encode(array(
					'type' => 3,
					'id' => 'partial-users-group',
					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'UsersGroup success deleted.').'</strong></div>',
			...
			$data .= '<div class="dialog-header">'.Yii::t('', 'Hapus UsersGroup').'</div>';
	...
	public function loadModel($id) {
		$model=UsersGroup::model()->findByPk($id);
		if($model===null)
	...
	protected function performAjaxValidation($model) {
		if(isset($_POST['ajax']) && $_POST['ajax']==='users-group-form') {
