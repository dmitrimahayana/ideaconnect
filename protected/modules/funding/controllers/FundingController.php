<?php
/* FundingController* Handle FundingController* Copyright (c) 2012, SWEVEL. All rights reserved.
* version: 0.0.1
* Reference start
*
* TOC :
...
class FundingController extends /*SBaseController*/ Controller
 {
	/**
	...
	public function actionIndex() {
		...
		$dataProvider=new CActiveDataProvider('Funding', array(
			//'criteria' => $criteria,
			//'pagination' => array(
			//	'size' => 10
			//)
	...
	public function actionAdminManage() {
		$model=new Funding('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Funding'])) {
			$model->attributes=$_GET['Funding'];
		}
	...
	public function actionAdminAdd() {
		...
		if(isset($_POST['Funding'])) {
			$model->attributes=$_POST['Funding'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'Funding success created.'));
	...
	public function actionAdminEdit($id) {
		...
		if(isset($_POST['Funding'])) {
			$model->attributes=$_POST['Funding'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'Funding success updated.'));
				$this->redirect(array('adminview','id'=>$model->id));
				//$this->redirect(array('adminmanage'));
			}
		}
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				...
				echo CJSON::encode(array(
					'type' => 3,
					'id' => 'partial-funding',
					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'Funding success deleted.').'</strong></div>',
			...
			$data .= '<div class="dialog-header">'.Yii::t('', 'Hapus Funding').'</div>';
	...
	public function loadModel($id) {
		$model=Funding::model()->findByPk($id);
		if($model===null)
	...
	protected function performAjaxValidation($model) {
		if(isset($_POST['ajax']) && $_POST['ajax']==='funding-form') {
