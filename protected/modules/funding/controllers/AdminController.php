...
class AdminController extends Controller {
    ...
    public function actionIndex() {
        ...
        $dataProvider=new CActiveDataProvider('FundingUser', array(
            //'criteria' => $criteria,
            //'pagination' => array(
            //	'size' => 10
            //)
    ...
    public function actionAdminAdd() {
        $model=new FundingUser;

        // Uncomment the following line if AJAX validation is needed
        $this->performAjaxValidation($model);

        if(isset($_POST['FundingUser'])) {
            $model->attributes=$_POST['FundingUser'];
            if($model->save()) {
                Yii::app()->user->setFlash('success', Yii::t('', 'FundingUser success created.'));
    ...
    public function actionAdminEdit($id) {
        ...
        if(isset($_POST['FundingUser'])) {
            $model->attributes=$_POST['FundingUser'];
            $model->varificator_id=Yii::app()->user->id_user;
            $model->verification_time=date("Y-m-d H:i:s");
            if($model->save()) {
                Yii::app()->user->setFlash('success', Yii::t('', 'FundingUser success updated.'));
                $this->redirect(array('adminview','id'=>$model->id));
                //$this->redirect(array('adminmanage'));
            }
            else {
        ...
        if(Yii::app()->request->isPostRequest) {
            ...
            if(isset($id)) {
                ...
                echo CJSON::encode(array(
                    'type' => 3,
                    'id' => 'partial-funding-user',
                    'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'FundingUser success deleted.').'</strong></div>',
            ...
            $data .= '<div class="dialog-header">'.Yii::t('', 'Hapus FundingUser').'</div>';
    ...
    public function loadModel($id) {
        $model=FundingUser::model()->findByPk($id);
        if($model===null)
    ...
    protected function performAjaxValidation($model) {
        if(isset($_POST['ajax']) && $_POST['ajax']==='funding-user-form') {
