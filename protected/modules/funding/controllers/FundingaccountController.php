<?php
/* FundingAccountController* Handle FundingAccountController* Copyright (c) 2012, SWEVEL. All rights reserved.
* version: 0.0.1
* Reference start
*
* TOC :
...
class FundingaccountController extends /*SBaseController*/ Controller
 {
	/**
	...
	public function actionIndex() {
		...
		$dataProvider=new CActiveDataProvider('FundingAccount', array(
			//'criteria' => $criteria,
			//'pagination' => array(
			//	'size' => 10
			//)
	...
	public function actionAdminManage() {
		$model=new FundingAccount('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['FundingAccount'])) {
			$model->attributes=$_GET['FundingAccount'];
		}
	...
	public function actionAdminAdd() {
		$model=new FundingAccount;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['FundingAccount'])) {
			$model->attributes=$_POST['FundingAccount'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'FundingAccount success created.'));
	...
	public function actionAdminEdit($id) {
		...
		if(isset($_POST['FundingAccount'])) {
			$model->attributes=$_POST['FundingAccount'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'FundingAccount success updated.'));
				$this->redirect(array('adminview','id'=>$model->id));
				//$this->redirect(array('adminmanage'));
			}
		}
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				...
				echo CJSON::encode(array(
					'type' => 3,
					'id' => 'partial-funding-account',
					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'FundingAccount success deleted.').'</strong></div>',
			...
			$data .= '<div class="dialog-header">'.Yii::t('', 'Hapus FundingAccount').'</div>';
	...
	public function loadModel($id) {
		$model=FundingAccount::model()->findByPk($id);
		if($model===null)
	...
	protected function performAjaxValidation($model) {
		if(isset($_POST['ajax']) && $_POST['ajax']==='funding-account-form') {
