<?php
/* BankController* Handle BankController* Copyright (c) 2012, SWEVEL. All rights reserved.
* version: 0.0.1
* Reference start
*
* TOC :
...
class BankController extends /*SBaseController*/ Controller
 {
	/**
	...
	public function actionIndex() {
		...
		$dataProvider=new CActiveDataProvider('Bank', array(
			//'criteria' => $criteria,
			//'pagination' => array(
			//	'size' => 10
			//)
	...
	public function actionAdminAdd() {
		...
		if(isset($_POST['Bank'])) {
			...
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'Bank success created.'));
	...
	public function actionAdminEdit($id) {
		...
		if(isset($_POST['Bank'])) {
			$model->attributes=$_POST['Bank'];
			if($model->save()) {
				Yii::app()->user->setFlash('success', Yii::t('', 'Bank success updated.'));
				$this->redirect(array('adminview','id'=>$model->id));
				//$this->redirect(array('adminmanage'));
			}
		}
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				...
				echo CJSON::encode(array(
					'type' => 3,
					'id' => 'partial-bank',
					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'Bank success deleted.').'</strong></div>',
			...
			$data .= '<div class="dialog-header">'.Yii::t('', 'Hapus Bank').'</div>';
