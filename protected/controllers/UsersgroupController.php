...
class UsersgroupController extends /*SBaseController*/ Controller
{
			...
			if(strlen($jsonError) > 2) {
				...
				if($model->save()) {
					echo CJSON::encode(array(
						'type' => 3,
						'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'UsersGroup success created.').'</strong></div>',
		...
		if(isset($_POST['UsersGroup'])) {
			...
			if(strlen($jsonError) > 2) {
				...
				if($model->save()) {
					echo CJSON::encode(array(
						'type' => 3,
						'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'UsersGroup success updated.').'</strong></div>',
						'get' => Yii::app()->controller->createUrl('adminmanage',array('type'=>'ajax'))
					));
				} else {
					print_r($model->getErrors());
	...
	public function actionAdminDelete($id) 
	{
		if(Yii::app()->request->isPostRequest) {
			...
			$data .= '<div class="dialog-header">Hapus UsersGroup</div>';
