<?php
/* MenutypesController* Handle MenutypesController* Copyright (c) 2012, SWEVEL. All rights reserved.
* version: 0.0.1
* Reference start
*
* TOC :
...
class MenutypesController extends /*SBaseController*/ Controller
{
	/**
	...
	public function actionAdminManage()	{
		$model=new MenuTypes('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['MenuTypes'])) {
			$model->attributes=$_GET['MenuTypes'];
		}
	...
	public function actionAdminAdd() {
		...
		if(isset($_POST['MenuTypes'])) {
			$model->attributes=$_POST['MenuTypes'];
			...
			if(strlen($jsonError) > 2) {
				...
				if($model->save()) {
					echo CJSON::encode(array(
						'type' => 3,
						'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'MenuTypes success created.').'</strong></div>',
		...
		if(isset($_POST['MenuTypes'])) {
			...
			if(strlen($jsonError) > 2) {
				...
				if($model->save()) {
					echo CJSON::encode(array(
						'type' => 3,
						'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'MenuTypes success updated.').'</strong></div>',
						'get' => Yii::app()->controller->createUrl('adminmanage',array('type'=>'ajax'))
					));
				} else {
					print_r($model->getErrors());
	...
	public function actionAdminDelete($id) {
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				...
				echo CJSON::encode(array(
					'type' => 3,
					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'MenuTypes success deleted.').'</strong></div>',
					'get' => Yii::app()->controller->createUrl('adminmanage',array('type'=>'ajax')),
				));
			}
			...
			$data .= '<div class="dialog-header">Hapus MenuTypes</div>';
	...
	public function loadModel($id) {
		$model=MenuTypes::model()->findByPk($id);
		if($model===null)
	...
	protected function performAjaxValidation($model) {
		if(isset($_POST['ajax']) && $_POST['ajax']==='menu-types-form')
		{
