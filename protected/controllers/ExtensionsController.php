<?php
/* ExtensionsController* Handle ExtensionsController* Copyright (c) 2012, SWEVEL. All rights reserved.
* version: 0.0.1
* Reference start
*
* TOC :
...
class ExtensionsController extends /*SBaseController*/ Controller
{
	/**
	...
	public function actionAdminManage()	{
		$model=new ComExtensions('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['ComExtensions'])) {
			$model->attributes=$_GET['ComExtensions'];
		}
	...
	public function actionAdminAdd() {
		$model=new ComExtensions;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['ComExtensions'])) {
			$model->attributes=$_POST['ComExtensions'];
			...
			if(strlen($jsonError) > 2) {
				...
				if($model->save()) {
					echo CJSON::encode(array(
						'type' => 3,
						'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'ComExtensions success created.').'</strong></div>',
		...
		if(isset($_POST['ComExtensions'])) {
			...
			if(strlen($jsonError) > 2) {
				...
				if($model->save()) {
					echo CJSON::encode(array(
						'type' => 3,
						'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'ComExtensions success updated.').'</strong></div>',
						'get' => Yii::app()->controller->createUrl('adminmanage',array('type'=>'ajax'))
					));
				} else {
					print_r($model->getErrors());
	...
	public function actionAdminDelete($id) {
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				...
				echo CJSON::encode(array(
					'type' => 3,
					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'ComExtensions success deleted.').'</strong></div>',
					'get' => Yii::app()->controller->createUrl('adminmanage',array('type'=>'ajax')),
				));
			}
			...
			$data .= '<div class="dialog-header">Hapus ComExtensions</div>';
	...
	public function loadModel($id) {
		$model=ComExtensions::model()->findByPk($id);
		if($model===null)
	...
	protected function performAjaxValidation($model) {
		if(isset($_POST['ajax']) && $_POST['ajax']==='com-extensions-form')
		{
