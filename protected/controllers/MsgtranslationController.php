<?php
/* MsgtranslationController* Handle MsgtranslationController* Copyright (c) 2012, SWEVEL. All rights reserved.
* version: 0.0.1
* Reference start
*
* TOC :
...
class MsgtranslationController extends /*SBaseController*/ Controller
{
	/**
	...
	public function actionAdminManage()	{
		$model=new MsgTranslation('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['MsgTranslation'])) {
			$model->attributes=$_GET['MsgTranslation'];
		}
	...
	public function actionAdminAdd() {
		$model=new MsgTranslation;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['MsgTranslation'])) {
			$model->attributes=$_POST['MsgTranslation'];
			...
			if(strlen($jsonError) > 2) {
				...
				if($model->save()) {
					echo CJSON::encode(array(
						'type' => 3,
						'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'MsgTranslation success created.').'</strong></div>',
		...
		if(isset($_POST['MsgTranslation'])) {
			...
			if(strlen($jsonError) > 2) {
				...
				if($model->save()) {
					echo CJSON::encode(array(
						'type' => 3,
						'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'MsgTranslation success updated.').'</strong></div>',
						'get' => Yii::app()->controller->createUrl('adminmanage',array('type'=>'ajax'))
					));
				} else {
					print_r($model->getErrors());
	...
	public function actionAdminDelete($id) {
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				...
				echo CJSON::encode(array(
					'type' => 3,
					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('', 'MsgTranslation success deleted.').'</strong></div>',
					'get' => Yii::app()->controller->createUrl('adminmanage',array('type'=>'ajax')),
				));
			}
			...
			$data .= '<div class="dialog-header">Hapus MsgTranslation</div>';
	...
	public function loadModel($id) {
		$model=MsgTranslation::model()->findByPk($id);
		if($model===null)
	...
	protected function performAjaxValidation($model) {
		if(isset($_POST['ajax']) && $_POST['ajax']==='msg-translation-form')
		{
